...
const useRenderScores = (otherSnakes, canvasWidth) => {
    return useCallback((ctx) => {
        ...
        const maxWidth = canvasWidth - 2 * padding; // Verfügbare Breite innerhalb des Spielfeldes
        ...
        Object.values(otherSnakes).forEach((snake) => {
            ...
            let nameWidth = ctx.measureText(`Player ${displayName}: ${score} points`).width;

            if (nameWidth > maxWidth) {
                while (nameWidth > maxWidth && displayName.length > 0) {
                    displayName = displayName.slice(0, -1);
                    nameWidth = ctx.measureText(`Player ${displayName + ellipsis}: ${score} points`).width;
                }
