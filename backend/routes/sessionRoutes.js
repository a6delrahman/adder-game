...
router.get('/sessions', (req, res) => {
    const sessions = Array.from(sessionService.getAllSessions().values());
    res.json(sessions);
});

// API: Aktuelle Spieler und GameStates abrufen
router.get('/gamestates', (req, res) => {
    const gameStates = Array.from(sessionService.gameStates.entries()).map(([id, state]) => ({
        sessionId: id,
        ...state
    }));
    res.json(gameStates);
...
router.get('/players', (req, res) => {
    const players = [];
    sessionService.gameStates.forEach((state) => {
...
router.get('/sessions/:sessionId', (req, res) => {
    ...
    const session = sessionService.getAllSessions().get(sessionId);

    if (!session) {
        return res.status(404).json({ message: 'Session not found' });
    }

    const gameState = sessionService.gameStates.get(sessionId);
